...
class TaskPlanner:
    ...
    def select_next_challenge(self, challenges: List[Challenge]) -> Optional[Challenge]:
        ...
        if not available:
            ...
            return None
        zone_order = [ChallengeZone.ZONE_1, ChallengeZone.ZONE_2, ChallengeZone.ZONE_3, ChallengeZone.ZONE_4]
        for zone in zone_order:
            zc = [c for c in available if c.zone == zone]
            if zc:
                zc.sort(key=lambda c: (self.failed_challenges.get(c.id, 0), -c.base_score))
                selected = zc[0]
                logger.info(f"Selected: [{selected.zone.value}] {selected.name} (ID: {selected.id})")
                return selected
        return None
